/**
  ...
  Caluculates the shortest distance to a given city from given input.
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void readCity( FILE *fp, struct City *cty ) {
  ...
  // ...
    fscanf(fp, "%s %lf %lf", cty->cityName, &cty->latitude, &cty->longitude);
...
/**
 ...
 @param argc nume of arguments, should be 2.
...
int main( int argc, char *argv[] )
{
  ...
  // ...
  struct City * cityPointer = (struct City *) malloc( n * sizeof(struct City));

  // Use the readCity function to fill in the fields of all the cities.

  // ...
  for (int i = 0; i < n; i++)
    readCity(fp, &cityPointer[i]);
  ...
  while ( scanf( "%lf%lf", &lat, &lon ) == 2 ) {
    ...
    // ...
    int index = 0;
    double temp = globalDistance(lat, lon, cityPointer[index].latitude, cityPointer[index].longitude);
      
    for (int i = 0; i < n; i++) {
      if (temp > globalDistance(lat, lon, cityPointer[i].latitude, cityPointer[i].longitude)) {
        temp = globalDistance(lat, lon, cityPointer[i].latitude, cityPointer[i].longitude);
        index = i;
      }
    }
      
      printf("%s %lf", cityPointer[index].cityName, temp);
  }
